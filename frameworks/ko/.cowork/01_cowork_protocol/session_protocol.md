...
1. 명확한 작업 요청이면 `project_state.md`에서 현재 제약을 확인하고 관련 문서만 읽어 바로 수행한다. 일반 브리핑과 작업 재선택은 생략하되, 트리거가 도래한 이월 항목은 한 줄로 알린다(§1D).
...
브리핑의 이월 항목은 `project_state.md`의 `이월 백로그`에서 가져온다. 현재 작업에 영향을 주는 항목을 간결하게 보여주고, 여러 건이면 표로 정리한다. 이월 트리거 감시는 AI의 책임이다 — Human이 찾아 지시하기 전에, 브리핑·작업 중 이 표를 대조해 도래한 트리거나 착수 가능한 항목을 AI가 먼저 꺼낸다. 작업이 지정된 세션에서는 표 전체를 보이지 않고 도래한 항목만 한 줄로 알린다.
