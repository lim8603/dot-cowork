...
- [ ] **라이브 상태 크기 점검 (F-05)** — UTF-8 전체 크기를 측정하고 R1/R2의 예산(공유 24 KiB / 개인 12 KiB 기본 경고선)·긴 행을 확인한다. 초과 시 제약과 근거를 보존하여 정리한다. 상세는 §공유 상태 인덱스 관리.
- [ ] **협업 실행 모드 확인 (F-06)** — `project_state.md`의 `협업 실행 모드`(solo/team)를 확인한다. solo면 역할별 부기를 생략하고 project_state 중심으로 운영하되 좌석 정의는 유지한다(§`decision_authority_matrix.md` 협업 실행 모드)
- [ ] 위 세션 시작 자체 점검(로그 생성·크기 예산·모드)은 자발적 준수에 의존하므로, 되돌리기 어려운 항목은 도구 훅으로 기계적 강제를 걸 수 있다(F-10, `tooling_environment_guide.md` §세션 규율 강제)
...
브리핑의 `💡 이전 이월` 항목은 `project_state.md`의 **`이월 백로그` 표(단일 SSOT)** 에서 가져온다. 이월이 여러 건이거나 트리거 대기 항목이 있으면 요약 한 줄로 줄이지 말고 **표를 그대로 브리핑에 포함**한다. 이월 트리거 감시는 AI의 책임이다 — Human이 찾아 지시하기 전에, 브리핑·작업 중 이 표를 대조해 도래한 트리거나 착수 가능한 항목을 **AI가 먼저 꺼낸다**.
...
- [ ] **게이트·불변식 변경 시 종단 재검증 (F-11)** — 이번 세션이 검증 규칙·게이트·불변식·계약을 조였다면, §2에서 적어둔 **소비자 목록**을 꺼내 종단 스위트(E2E·통합)를 재실행하고 결과를 소비자별로 보고한다. 단위 테스트 통과는 이 항목을 대체하지 못한다 — 조인 판정의 낙진은 대개 조인 코드가 아니라 **그 관문을 지나는 다른 경로**에서 터진다. 실행하지 못했다면 **무엇이 미검증인지**를 세션 로그와 `project_state.md` 이월에 명시 등재한다(침묵 금지)
- [ ] `project_state.md` 동기화 — 현재 값 교체·짧은 완료 요약·상세 이관·크기 재측정(R1/R2).
- [ ] `project_state.md`의 `이월 백로그` 표 동기화 — 해소분 내리고 신규 이월/트리거 올림
- [ ] 개인 `my_state.md` 갱신 — 다음 시작점, 관련 Milestone, 담당 작업 상태, 이월 항목, 참조 세션 로그 (완료 서사 초과분 하베스트 — R1)
