...
진입점에는 안정적인 프로젝트 식별·문서 경로·도구별 지시를 둔다. Phase, 활성 Task, 다음 Milestone, 배포 버전은 `project_state.md`에서 읽으며 진입점에 복제하지 않는다. 실제 기술 스택은 `tech_stack.md`를 기준으로 한다. 재개 시 진입점 요약이 기준 문서와 충돌하면 근거를 확인해 오래된 요약을 링크로 교체한다.
...
| 체크 | 근거 규칙 | 강제 아이디어 |
|------|-----------|---------------|
| 세션 로그 생성 여부 | `session_protocol.md` §자동 기록 1 | 세션 시작 훅이 당일 `session_logs/session_*.md` 존재를 확인, 없으면 생성/경고 |
| 라이브 상태 문서 크기 예산 | F-05 | 훅이 `project_state.md`+`my_state.md` 크기를 측정, 예산 초과 시 R1/R2 하베스트 권고 표시 |
| 이월 백로그 표 브리핑 포함 | F-02 | 브리핑 산출물에 `이월 백로그` 표 존재를 린트 |
