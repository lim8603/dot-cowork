...
| Feature | What it does | Trigger | Primary artifacts | Expected outcome |
| --- | --- | --- | --- | --- |
| Session state restore | Finds current constraints and next actions for the requested work | At work start | Root entrypoint file, `06_evolution/project_state.md`, relevant documents | Human and assistant work from the current documented state |
| Session briefing | Presents active work and carryover items briefly when the user has not specified a task | Only when a task choice is needed | `06_evolution/project_state.md`, relevant active registries | The user can choose work without a document dump |
| Task-aware loading | Opens registries, canonical documents, and active details needed for the current task | After the task or work choice is known | `06_evolution/project_state.md`, relevant source documents | Reading scope follows the work rather than the whole phase |
...
| Session log bootstrap | Opens a session log when substantive work needs a continuity record | When decisions, changes, or handoff details need recording | `members/<name>/workspace/session_logs/`, `06_evolution/templates/session_log_template.md` | Read-only requests do not create log files |
| Session log ignore hygiene | Checks the ignore rule for local logs | When creating the first session log | `.gitignore`, session log path rules | Logs do not create noisy repository diffs |
| Pending proposal surfacing | Detects unresolved proposals that affect the requested work or next work choice | When a related proposal is pending | `members/*/proposals/`, authority rules | Related approval debt is visible before dependent work proceeds |
...
| Intent-based workflow routing | Maps requested actions such as handoff, Proposal submission, phase transition, deliverable generation, and upgrade into procedures | When the user requests an action in context | `01_cowork_protocol/session_protocol.md`, proposal paths, gate docs, delivery and upgrade docs | Mentioning a keyword alone does not mutate the project |
| Change proposal capture | Turns a requested Change Proposal into a structured artifact | When the user asks to submit a shared-area change as a Proposal | `members/*/proposals/`, proposal templates, session log | Proposed changes remain reviewable and traceable |
...
| Context quality handoff | Reopens sources when earlier constraints are lost and suggests a handoff if continuity cannot be restored | When actual context errors persist after rereading | Session log, `project_state.md`, end-of-session protocol | Long-running work can be handed off with a clear next action |
